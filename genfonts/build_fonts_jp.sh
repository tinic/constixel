...
for font in "$FONT_DIR"/*.{ttf,otf}; do
  ...
  for size in "${SIZES[@]}"; do
    ./fontbm/build/fontbm --font-file "$font" --font-size "$size" --output "build/${base}_jp_${size}" --chars 32-128,12352-12447,12448-12543,12288-12351 --texture-crop-width --texture-crop-height --tabular-numbers
    ./build/genfont --font-file "build/${base}_jp_${size}.fnt" --out-dir ../fonts
    ./fontbm/build/fontbm --font-file "$font" --font-size "$size" --output "build/${base}_jp_${size}" --monochrome --texture-crop-width --texture-crop-height --tabular-numbers
    ./build/genfont --font-file "build/${base}_jp_${size}.fnt" --out-dir ../fonts
  done
done
